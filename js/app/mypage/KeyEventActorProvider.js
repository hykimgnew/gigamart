...
function fn_popFavDel() {
    ...
    // requestMyStatus = '';
    // console.log("myFavList 실행------------------------------>");
    // Gigamart.app.shopper_bag.KeyEventActorProvider.myFavList();
}
//찜한상품 선택삭제할 상품 없습니다. 토스트 팝업닫기
function fn_popFavDelNon() {
    $('#pop_favDel_non').hide();
}
//주문내역 없습니다. 토스트 팝업닫기
...
//마이 주문내역
// 마이 찜한상품
function makeMyOrderProduct() {

    var appendHtml =  '<tr name="ord_menu">';
        appendHtml += '    <td class="bdr_right" name="ord_date"></td>';
        appendHtml += '    <td class="bdr_right" name="ord_num"></td>';
        appendHtml += '    <td class="bdr_right" name="ord_info"></td>';
        appendHtml += '    <td class="bdr_right" name="ord_state"></td>';
        appendHtml += '    <td name="ord_name"></td>';
        appendHtml += '</tr>';

        // <tr name="ord_menu"><!-- <tr class="focus"> -->
        //     <td class="bdr_right" name="ord_date"><!-- 2015.05.06 14:06 --></td>
        //     <td class="bdr_right" name="ord_num"><!-- asdkgjlskd0020394 --></td>
        //     <td class="bdr_right" name="ord_info"><!-- [아침마루] 싱싱한 토마토 5입 1봉 외              … 3건 --></td>
        //     <td class="bdr_right" name="ord_state"><!-- 배송중 --></td>
        //     <td name="ord_name"><!-- 김이슬 --></td>
        // </tr>

    $('#ord_wrap').after(appendHtml);
}
...
App.defineClass('Gigamart.app.shopper_bag.KeyEventActorProvider', {
    _construct: function() {
        ...
        console.log("requestMyCnt : " + requestMyCnt+"개");
        if(requestMyStatus != null && requestMyStatus != '' && requestMyStatus != 'undefined') {
            if(requestMyStatus == 2){
                //찜한상품 페이지로
                $('li[name="nm_menu"]').eq(0).removeClass('focus');
                currentFocusMenu = 1;
                $('div[name="view_my"]').hide();
                myView = 2;
                currentFavListPage = requestMyPage;
                favFocusMenu = requestMyMenu;
                currentPageCnt = requestMyCnt;
                if(favFocusMenu ==0 || favFocusMenu ==1 || favFocusMenu == 2 || favFocusMenu == 3){
                    horizonFocus = favFocusMenu;
                    verticalFocus = 0;
                }else{
                    horizonFocus = Number(favFocusMenu)-4;
                    verticalFocus = 1;
                }
                console.log("horizonFocus : "+horizonFocus+",verticalFocus : "+verticalFocus+", favFocusMenu(메뉴위치) : "+favFocusMenu+", currentFavListPage123123(현페이징) : "+currentFavListPage+", currentPageCnt(현페이지상품갯수) : "+currentPageCnt)
                $('div[name="view_fav"]').show();
                requestMyStatus = '';
                this.myFavList();
                ...
                myView = 3;
                newFocusList = 0;
                // currentNewListPage = requestMyPage;
                // newFocusMenu = requestMyMenu;
                // currentNewPageCnt = requestMyCnt;

                // if(newFocusMenu ==0 || newFocusMenu ==1 || newFocusMenu == 2 || newFocusMenu == 3){
                //     horizonNewFocus = newFocusMenu;
                //     verticalNewFocus = 0;
                // }else{
                //     horizonNewFocus = Number(newFocusMenu)-4;
                //     verticalNewFocus = 1;
                // }
    ...
    getKeyEventActor: function (keyCode) {
            ...
            if (keyCode === global.VK_ENTER) {
                ...
                if(chgVolumeFocus == 0) {
                    ...
                    if(cartFocus == 0) {
                        ...
                        location.href = EXHB_PATH + 'order.html?SHOPPER_STATUS=' + SHOPPER_STATUS + '&requestOrderScreen=mypage.html' + '&userID='+ userID + '&userName=' + userName;
                    }
                    ...
                    if(cartFocus == 1) {
                        ...
                        if($('#ec_list > li[name="ec_li_list"]').size() > 0) {
                            ...
                            $('#span_videoDiv video').remove();
                            appConfiguration.localSystem.mute = true; // 음소거 설정
                            location.href = EXHB_PATH + 'order.html?SHOPPER_STATUS=' + SHOPPER_STATUS + '&requestOrderScreen=mypage.html'+ '&userID='+ userID + '&userName=' + userName + '&startScreen=2';   
                        } else {
        ...
        // *****************************************************************************
            ...
            if(keyCode === global.VK_RED) {
                /** 기획전으로 이동 */
                location.href ="exhb.html?SHOPPER_STATUS=" + SHOPPER_STATUS+ '&userID='+ userID + '&userName=' + userName;
            }
            ...
            if(keyCode === global.VK_PLAY) {
                /** 전체 카테고리로 이동 */
                location.href = "category.html?SHOPPER_STATUS=" + SHOPPER_STATUS+ '&userID='+ userID + '&userName=' + userName;
            }
            ...
            if (keyCode === global.VK_ENTER) {
                ...
                if(myView == 0){
                    ...
                    if(currentFocusList == 0){
                        ...
                        if(currentFocusMenu == 0){
                            location.href = EXHB_PATH + 'order.html?SHOPPER_STATUS=' + SHOPPER_STATUS+ '&userID='+ userID + '&userName=' + userName;
                        }
                        //찜한상품
                    ...
                    //전체선택 
                        ...
                        //버튼
                            ...
                            //편집모드일때
                                ...
                                for(var a = 0; a<currentPageCnt ; a++){
                                    curenterHtml2 = $('li[name="fav_menu"]').eq(a).children().children('.all_chk').attr('src');
                                    if(curenterHtml2 == "../images/checkbox_sel.png"){
                                        chkCnt2 = chkCnt2 + 1;
                                    }
                                }
                                console.log("chkCnt 갯수 : "+chkCnt2);
                                console.log("currentPageCnt 갯수 : "+currentPageCnt);
                                ...
                                //삭제할상품없을때
                                if(favDelYn == 0){
                                    console.log("삭제할 상품없음.");
                                    $('#pop_favDel_non').show();
                                    setTimeout("fn_popFavDelNon()", 2000);
                                }else if(favDelYn == 1){
                                   for(var i=0; i<currentPageCnt; i++){
                                   ...
                                   console.log("삭제 완료->팝업닫고 fav 다시호출");
                                    $('div[name="fav_allSelect"]').removeClass('focus');
                                    $('span[name="fav_btn_ud"]').removeClass('focus');
                                    $('.all_chkA').attr('src','../images/checkbox.png');
                                    favFocusList = 1;
                                ...
                                //         arrPdIdList[i] = productId;
                                //         console.log("선택된 pd i : "+i+" ,id : "+productId);
                                //         favDelYn = 1;
                                //         //this.deleteFavProduct(productId,i,chkCnt2);
                                //         this.deleteFavProduct(productId);
                                //     }else{
                                //         console.log("선택삭제할 상품없음");
                                //         favDelYn = 0;
                ...
                //자주구매한 상품 - 최근본상품
                    ...
                    if(newFocusList == 0){
                        ...
                        location.href = EXHB_PATH + "product1.html?id="+pd_id+"&categoryDtlCode="+cate_id+"&SHOPPER_STATUS=" + SHOPPER_STATUS+"&requestMyStatus=" + MY_STATUS+"&requestMyPage=" + currentNewListPage+"&requestMyMenu=" + newFocusMenu+"&requestMyCnt=" + currentNewPageCnt+ '&userID='+ userID + '&userName=' + userName;
                ...
                //주문내역 상세2
                    ...
                    //버튼영역
                        ...
                        //주문내역 상세2 -> 주문내역
                            ...
                            else if(orderDt2BtnFocus == 1){
                                console.log("주문취소팝업띄우기");
                                orderDt2FocusList = 2;
                                $('#pop_order_ref').show();
                                orderDt2RefBtn = 0;
                                $('span[name="ord_ref_ok"]').addClass('focus');
                            }
                        }  
                    }
                    //팝업일때
                     else if(orderDt2FocusList == 2){
                        //확인일때 주문취소 
                        if(orderDt2RefBtn == 0){
                            $('span[name="ord_ref_ok"]').removeClass('focus');
                            orderDt2RefBtn = 0;
                            $('#pop_order_ref').hide(); 
                            var ordId = $('td[name="order_dt_num"]').html();
                            console.log("주문취소go  ordId : "+ordId);
                            this.orderRefProduct(ordId)
                        }
                        //취소일때 팝업 hide
                        else if(orderDt2RefBtn == 1){
                            $('span[name="ord_ref_cancel"]').removeClass('focus');
                            orderDt2RefBtn = 0;
                            orderDt2FocusList = 1;
                            $('#pop_order_ref').hide();  
                        }
                ...
                if(keyCode === global.VK_UP) {
                    ...
                    if(myView == 0){
                        ...
                        //버튼
                            ...
                            //주문내역이 없으면 상단 장바구니 버튼으로 이동
                            if($('tr[name="ord_menu"]').length == 0){
                                ordBtnFocus = false;
                                currentFocusList = 0;
                                $('li[name="nm_menu"]').eq(currentFocusMenu).addClass('focus');   
                            }
                    ...
                    //세번째 화면 -찜한상품
                        ...
                        //상품목록
                            ...
                            // 첫번째 행 일때
                            if(verticalFocus == 0) {
                                console.log("첫번째행일때 prevPageYN : "+prevPageYN);
                                //편집모드아닐때
                                if(favUpdateMode == false){
                                    if(prevPageYN == true) {
                                        // 전 페이지 조회
                                        console.log("### 상세카테고리 : 전 페이지 조회");
                                        currentFavListPage = Number(currentFavListPage) - 1;
                                        ...
                                        verticalFocus   = Number(verticalFocus) + 1;    // 행 증가(전페이지의 마지막으로 위치)
                                        horizonFocus    = horizonFocus;         // 열 증감 없음
                                        favFocusMenu    = Number(favFocusMenu) + 4;   // 위치 변경
                                        this.updateSubCategoryList();                // 페이지 변경
                                        //updateProductInfo();                    // 우측 상품 정보 갱신
                                        if(requestMyStatus != null && requestMyStatus != '' && requestMyStatus != 'undefined' && requestMyStatus != 0) { 
                                            console.log("11111111111111111111111111111111111111111111111111111111111111111111111111위로이동");
                                        }else {
                                            console.log("2222222222222222222222222222222222222222222222222222222222222222222222222위로이동");
                            ...
                            // 두번째 행 일때
                                ...
                                console.log("두번째행일때 favUpdateMode : "+favUpdateMode);
                                //편집모드아닐때
                                if(favUpdateMode == false){
                                    console.log("두번째 행 일때");
                                    $('li[name="fav_menu"]').eq(favFocusMenu).removeClass('focus');
                                    $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').empty();
                                    verticalFocus   = Number(verticalFocus) - 1;    // 행 감소
                                    horizonFocus    = horizonFocus;         // 열 증감 없음
                                    favFocusMenu    = Number(favFocusMenu) - 4;  // 위치 변경
                                    //updateProductInfo();                    // 우측 상품 정보 갱신
                                    $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                        ...
                        if(newFocusList == 0){
                            ...
                            if(verticalNewFocus == 0) {
                                if(prevNewPageYN == true) {
                                    ...
                                    verticalNewFocus   = Number(verticalNewFocus) + 1;      // 행 증가(전페이지의 마지막으로 위치)
                                    horizonNewFocus    = horizonNewFocus;           // 열 증감 없음
                                    newFocusMenu       = Number(newFocusMenu) + 4;             // 위치 변경
                                    //updateProductInfo();                          // 우측 상품 정보 갱신
                            ...
                            // 두번째 행 일때
                                ...
                                verticalNewFocus   = Number(verticalNewFocus) - 1;    // 행 감소
                                horizonNewFocus    = horizonNewFocus;         // 열 증감 없음
                                newFocusMenu       = Number(newFocusMenu) - 4;  // 위치 변경
                                //updateProductInfo();                    // 우측 상품 정보 갱신
                                $('li[name="new_menu"]').eq(newFocusMenu).addClass('focus');
                    ...
                    //취소/환불내역 상세2
                        ...
                        if(currentRefDtListPage == 0){
                            ...
                            //리스트1개일때 1행->이전페이지
                                ...
                                if(refDtTotalPageYN == true){
                                    console.log("리스트갯수 1개이고 refDtTotalPageYN true일때 refDt2FocusMenu : "+refDt2FocusMenu);
                                    
                                    if(refDt2FocusList == 0){
                                        if(refDt2FocusMenu == 0){
                                            console.log("리스트1개일때 1행->전페이지");
                                    ...
                                    //버튼영역
                                        ...
                                        refDt2FocusList = 0;
                                        refDt2BtnFocus =0;
                                        $('span[name="ref_dt2_cancel"]').removeClass('focus');
                                        $('span[name="ref_dt2_back"]').removeClass('focus');
                                        //버튼->마지막줄
                            ...
                            //리스트 2개일때 
                                ...
                                if(refDtTotalPageYN == true){
                                    console.log("리스트갯수 2개이고 refDtTotalPageYN true일때 refDt2FocusMenu : "+refDt2FocusMenu);
                                    //환불내역 리스트
                                    if(refDt2FocusList == 0){
                                        if(refDt2FocusMenu == 0){
                                            console.log("리스트2개일때 1행->전페이지");
                                            refDtTotalPageYN = false;
                                            refDt2FocusMenu = 2;
                                            this.pagingRefDtProduct(Number(currentRefDtListPage)-1);
                                        }
                    ...
                    //주문내역 상세2
                        ...
                        if(currentOrderDtListPage == 0){
                            ...
                            //리스트1개일때 1행->이전페이지
                                ...
                                if(orderDtTotalPageYN == true){
                                    console.log("리스트갯수 1개이고 orderDtTotalPageYN true일때 orderDt2FocusMenu : "+orderDt2FocusMenu);
                                    //주문내역 리스트
                                    if(orderDt2FocusList == 0){
                            ...
                            //리스트 2개일때 
                                ...
                                if(orderDtTotalPageYN == true){
                                    //주문내역 리스트
                                    if(orderDt2FocusList == 0){
                                        console.log("리스트갯수 2개이고 orderDtTotalPageYN true일때 orderDt2FocusMenu : "+orderDt2FocusMenu);
                                        if(orderDt2FocusMenu == 0){
                                            console.log("리스트2개일때 1행->전페이지");
                                            orderDtTotalPageYN = false;
                                            orderDt2FocusMenu = 2;
                ...
                if(keyCode === global.VK_DOWN) {
                   //첫번째 화면-마이페이지
                    if(myView == 0){
                        ...
                        if(currentFocusList == 0){
                            $('li[name="nm_menu"]').eq(currentFocusMenu).removeClass('focus');   
                                //주문내역이 없을땐 이용안내/공지사항/자주묻는질문 버튼으로 이동
                                console.log("길이!!!!!!!!!!!"+$('tr[name="ord_menu"]').length);
                                if($('tr[name="ord_menu"]').length == 0){
                                    currentFocusList = 2
                        ...
                        //주문내역
                            ...
                            //주문내역 1행->2행 / 1행만 있을경우->이용안내버튼 go
                            else if(ordBtnFocus == false && currentFocusMenu2 == 0){
                                $('tr[name="ord_menu"]').eq(currentFocusMenu2).removeClass('focus');  
                                $('tr[name="ord_menu"]').eq(currentFocusMenu2).children('.bdr_right').css("border-right","1px solid #cfcfcf");    
                                if($('tr[name="ord_menu"]').length == 1){
                                    currentFocusList = 2
                                    $('li[name="nm_menu2"]').eq(currentFocusMenu3).addClass('focus');
                                }else{
                                    currentFocusMenu2 = Number(currentFocusMenu2) + 1;
                            ...
                            //주문내역 2행->3행 / 2행만 있을경우->이용안내버튼 go
                            else if(ordBtnFocus == false && currentFocusMenu2 == 1){
                                $('tr[name="ord_menu"]').eq(currentFocusMenu2).removeClass('focus');
                                $('tr[name="ord_menu"]').eq(currentFocusMenu2).children('.bdr_right').css("border-right","1px solid #cfcfcf");  
                                    
                                if($('tr[name="ord_menu"]').length == 2){
                                    currentFocusList = 2
                                    $('li[name="nm_menu2"]').eq(currentFocusMenu3).addClass('focus');
                                }else{
                                    currentFocusMenu2 = Number(currentFocusMenu2) + 1;
                        ...
                        //상품목록
                            ...
                            if(verticalFocus >= 0 && verticalFocus < 1) {
                                ...
                                if(favUpdateMode == false){
                                    ...
                                    if(Number(currentPageCnt) - 4 <= favFocusMenu) {
                                    ...
                                    verticalFocus   = Number(verticalFocus) + 1;    // 행 증가
                                    horizonFocus    = horizonFocus;         // 열 증감 없음
                                    favFocusMenu    = Number(favFocusMenu) + 4;     // 위치 변경
                                    //updateProductInfo();                    // 우측 상품 정보 갱신
                            ...
                            // 
                                ...
                                if(favUpdateMode == false){
                                    ...
                                    if(nextPageYN == true) {
                                        ...
                                        if(currentPageCnt <= Number(favFocusMenu) + 1) {
                                            console.log("%%%%%%아래로 이동했을때 아래 위치에 상품이 없으면 ");
                                            favFocusMenu = Number(currentPageCnt) - 1;
                                            horizonFocus = Number(currentPageCnt) - 1;
                                        }
                                       ...
                                       //updateProductInfo();                    // 우측 상품 정보 갱신
                                        ...
                                        if(requestMyStatus != null && requestMyStatus != '' && requestMyStatus != 'undefined' && requestMyStatus != 0) { 
                                            console.log("11111111111111111111111111111111111111111111111111111111111111111111111111아래로이동");
                                        }else {
                                            console.log("22222222222222222222222222222222222222222222222222222222222222222222222222아래로이동");
                                            $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                                            $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').append(btnokfill);    
                                        }
                    ...
                    //최근 본상품
                        ...
                        if(newFocusList == 0){
                            ...
                            if(verticalNewFocus >= 0 && verticalNewFocus < 1) {
                                ...
                                verticalNewFocus   = Number(verticalNewFocus) + 1;   // 행 증가
                                horizonNewFocus    = horizonNewFocus;        // 열 증감 없음
                                newFocusMenu       = Number(newFocusMenu) + 4;       // 위치 변경
                                //updateProductInfo();                       // 우측 상품 정보 갱신
                            ...
                            // 두번째 행 일때
                                ...
                                if(nextNewPageYN == true) {
                                    ...
                                    currentNewListPage = Number(currentNewListPage) + 1;
                                    this.updateSubCategoryNewList();    // 페이지 변경
                                    verticalNewFocus   = Number(verticalNewFocus) - 1;    // 행 감소
                                    horizonNewFocus    = horizonNewFocus;         // 열 증감 없음
                                    newFocusMenu    = Number(newFocusMenu) - 4;  // 위치 변경
                                    ...
                                    if(currentNewPageCnt <= newFocusMenu + 1) {
                                        console.log("%%%%%%아래로 이동했을때 아래 위치에 상품이 없으면 ");
                                        newFocusMenu = Number(currentNewPageCnt) - 1;
                                        horizonNewFocus = Number(currentNewPageCnt) - 1;
                                    }
                            ...
                            if(orderDt2FocusMenu == 0){
                                ...
                                //리스트갯수 1개일때
                                    ...
                                    if(orderDtTotalPageYN  == false){
                                       orderDt2FocusMenu = 0;
                                       this.pagingOrderedDtProduct(Number(currentOrderDtListPage)+1);     
                                    }else if(orderDtTotalPageYN == true){
                                        console.log("total페이지이므로 버튼으로 이동해야됨");
                                        $('tr[name="order_dt_menu2"]').eq(orderDt2FocusMenu).removeClass('focus');
                                        $('tr[name="order_dt_menu2"]').eq(orderDt2FocusMenu).children('td').css("border-right","1px solid #dedede");
                        ...
                        //상품목록
                            ...
                            if(favUpdateMode == false){
                                ...
                                if(horizonFocus >= 1 && horizonFocus < 4) {
                                    ...
                                    horizonFocus    = Number(horizonFocus) - 1;     // 열 감소
                                    favFocusMenu    = Number(favFocusMenu) - 1;  // 위치 변경
                                    //updateProductInfo();                    // 우측 상품 정보 갱신
                    ...
                    //최근 본상품
                        ...
                        if(newFocusList == 0){
                            ...
                            if(horizonNewFocus >= 1 && horizonNewFocus < 4) {
                                ...
                                horizonNewFocus    = Number(horizonNewFocus) - 1;     // 열 감소
                                newFocusMenu       = Number(newFocusMenu) - 1;  // 위치 변경
                                //updateProductInfo();                    // 우측 상품 정보 갱신
                    ...
                    //세번째화면 찜한상품
                        ...
                        //상품목록
                            ...
                            if(horizonFocus >= 0 && horizonFocus < 3) {
                                // 우측에 상품이 있을때
                                if(currentPageCnt > Number(favFocusMenu)+1) {
                                    console.log("!@########!@#!@# currentPageCnt: "+currentPageCnt+",favFocusMenu")
                                    //편집모드아닐때
                                    if(favUpdateMode == false){
                                        $('li[name="fav_menu"]').eq(favFocusMenu).removeClass('focus');
                                        $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').empty();
                                        ...
                                        verticalFocus   = verticalFocus;        // 행 증감 없음
                                        horizonFocus    = Number(horizonFocus) + 1;     // 열 증가
                                        favFocusMenu    = Number(favFocusMenu) + 1;  // 위치 변경
                                        //updateProductInfo();      
                                        $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                                        $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').append(btnokfill);
                        ...
                        if(newFocusList == 0){
                            ...
                            if(horizonNewFocus >= 0 && horizonNewFocus < 3) {
                                ...
                                if(currentNewPageCnt > newFocusMenu+1) {
                                    ...
                                    verticalNewFocus   = verticalNewFocus;        // 행 증감 없음
                                    horizonNewFocus    = Number(horizonNewFocus) + 1;     // 열 증가
                                    newFocusMenu    = Number(newFocusMenu) + 1;  // 위치 변경
                                    //updateProductInfo();      
                                    $('li[name="new_menu"]').eq(newFocusMenu).addClass('focus');
                                    $('li[name="new_menu"]:eq('+ newFocusMenu + ') > .dm_bdr').append(btnokfill);
                    ...
                    //주문내역 상세2
                        ...
                        //팝업영역
                            ...
                            if(orderDt2RefBtn == 0){
                                $('span[name="ord_ref_ok"]').removeClass('focus');
                                orderDt2RefBtn = 1;
                                $('span[name="ord_ref_cancel"]').addClass('focus');
                            }
                        }
                    }
    ...
    selectMyOrderList: function() {
        ...
        $.ajax({
            ...
            success     : function(result) {
                ...
                //결과값 없을때 0
                if(obj_length == 1){
                    $('#ord_wrap').after('<tr style="height : 153px;"><td colspan="5">주문내역이 없습니다.</td></tr>');
                    //주문내역 없습니다. 토스트메세지
                ...
                //결과값 있을때
                    ...
                    var resultObj = result['arr_order'];
                    var resultLen = Object.keys(resultObj).length;
                    ...
                    console.log("resultLen-->"+resultLen);
                            if(resultLen == 1){
                               ...
                               console.log("갯수 3개뿌리기");
                                for(var a =0; a<3; a++){
                                    makeMyOrderProduct();
                                } 
                            }
                        for(var z = 0; z<resultLen; z++){
                            if(z ==0){
                            var obj0 = result['arr_order'];
                            $('td[name="ord_date"]').eq(0).empty().append(obj0[0].order_date);
                            $('td[name="ord_num"]').eq(0).empty().append(obj0[0].order_id);
                            $('td[name="ord_cost"]').eq(0).empty().append(cn_toPrice(obj0[0].ordered_cost)+"원");
                            $('td[name="ord_state"]').eq(0).empty().append(obj0[0].status);
                            $('td[name="ord_name"]').eq(0).empty().append(obj0[0].shopper_id);
                                var obj = obj0[0].ordered_product[0];
                                console.log("i는 : "+0+" , obj : "+JSON.stringify(obj));
                                //상품정보 있을때
                                if(obj != undefined){
                                    $.each(obj0[0].ordered_product, function(pindex, pentry) {
                                        var obj = obj0[0].ordered_product;
                                        var obj_length = Object.keys(obj).length;
                                        console.log("obj_length---->"+obj_length);
                                        if(pindex == 0){
                                            $('td[name="ord_info"]').eq(0).empty().append(pentry['name'] +"외 ……"+obj_length+"건");
                                        }
                                    });
                                }
                                //상품정보 없을때
                                else{
                                    $('td[name="ord_info"]').eq(0).empty().append("없음");
                                }
                            }else if(z == 1){
                                var obj1 = result['arr_order'];
                                $('td[name="ord_date"]').eq(1).empty().append(obj1[1].order_date);
                                $('td[name="ord_num"]').eq(1).empty().append(obj1[1].order_id);
                                $('td[name="ord_cost"]').eq(1).empty().append(cn_toPrice(obj1[1].ordered_cost)+"원");
                                $('td[name="ord_state"]').eq(1).empty().append(obj1[1].status);
                                $('td[name="ord_name"]').eq(1).empty().append(obj1[1].shopper_id);
                                    var obj = obj1[1].ordered_product[0];
                                    console.log("i는 : "+1+" , obj : "+JSON.stringify(obj));
                                    //상품정보 있을때
                                    if(obj != undefined){
                                        $.each(obj1[1].ordered_product, function(pindex, pentry) {
                                            var obj = obj1[1].ordered_product;
                                            var obj_length = Object.keys(obj).length;
                                            console.log("obj_length---->"+obj_length);
                                            if(pindex == 0){
                                                $('td[name="ord_info"]').eq(1).empty().append(pentry['name'] +"외 ……"+obj_length+"건");
                                            }
                                        });
                                    }
                                    //상품정보 없을때
                                    else{
                                        $('td[name="ord_info"]').eq(1).empty().append("없음");
                                    }
                            }
                            else if(z == 2){
                                var obj2 = result['arr_order'];
                                $('td[name="ord_date"]').eq(2).empty().append(obj2[2].order_date);
                                $('td[name="ord_num"]').eq(2).empty().append(obj2[2].order_id);
                                $('td[name="ord_cost"]').eq(2).empty().append(cn_toPrice(obj2[2].ordered_cost)+"원");
                                $('td[name="ord_state"]').eq(2).empty().append(obj2[2].status);
                                $('td[name="ord_name"]').eq(2).empty().append(obj2[2].shopper_id);
                                    var obj = obj2[2].ordered_product[0];
                                    console.log("i는 : "+2+" , obj : "+JSON.stringify(obj));
                                    //상품정보 있을때
                                    if(obj != undefined){
                                        $.each(obj2[2].ordered_product, function(pindex, pentry) {
                                            var obj = obj2[2].ordered_product;
                                            var obj_length = Object.keys(obj).length;
                                            console.log("obj_length---->"+obj_length);
                                            if(pindex == 0){
                                                $('td[name="ord_info"]').eq(2).empty().append(pentry['name'] +"외 ……"+obj_length+"건");
                                            }
                                        });
                                    }
                                    //상품정보 없을때
                                    else{
                                        $('td[name="ord_info"]').eq(2).empty().append("없음");
                                    }
                            }  
                        }
        ...
        $.ajax({
            ...
            success     : function(result) {
                ...
                if(obj_length == 1){
                    ...
                    if(resultLen > 8) { 
                        for(var i=0; i<8; i++){
                            console.log("결과값이 8보다 크면 : i : "+i);
                            makeFavProduct();  
                            nextPageYN = true;
                            var obj = result['favorite'];
                            console.log("i : "+i+",,,,name : "+obj[i].name);
                            $.each(result['favorite'], function(index, entry) {
                                $('li[name="fav_img"]').eq(i).empty().append('<img src="' + obj[i].img + '" height="92" width="162" />');
                                $('li[name="fav_tit"]').eq(i).empty().append(obj[i].name);
                                $('li[name="fav_price"]').eq(i).empty().append(cn_toPrice(obj[i].cost) +"원");
                                $('input[name="fav_pd_id"]').eq(i).val(obj[i].product_id);
                    ...
                    else if(resultLen < 8 || resultLen == 8){
                        for(var i=0; i<resultLen; i++){
                            console.log("결과값이 8보다 작거나 8인경우 : i : "+i);
                            makeFavProduct(); 
                            var obj2 = result['favorite'];
                            console.log("i : "+i+",,,,name : "+obj2[i].name);
                            $.each(result['favorite'], function(index, entry) {
                                $('li[name="fav_img"]').eq(i).empty().append('<img src="' + obj2[i].img + '" height="92" width="162" />');
                                $('li[name="fav_tit"]').eq(i).empty().append(obj2[i].name);
                                $('li[name="fav_price"]').eq(i).empty().append(cn_toPrice(obj2[i].cost) +"원");
                                $('input[name="fav_pd_id"]').eq(i).val(obj2[i].product_id);
                    ...
                    if(totalFavListPage_html < 10 && totalFavListPage_html>0){
                    ...
                    if(requestMyStatus != null && requestMyStatus != '' && requestMyStatus != 'undefined' && requestMyStatus != 0) {
                       for(var b = 0; b<8; b++){
                            console.log("뭐냐고!!!!!bbb"+b);
                            $('li[name="fav_menu"]:eq('+ b + ') > .dm_bdr').empty();
                       }
                       console.log("123123::::"+requestMyStatus);
                       console.log("horizonFocus : "+horizonFocus+",verticalFocus : "+verticalFocus+", favFocusMenu(메뉴위치) : "+favFocusMenu+", currentFavListPage(현페이징) : "+currentFavListPage+", currentPageCnt(현페이지상품갯수) : "+currentPageCnt)
                
                        currentFavListPage = currentFavListPage;
                        favFocusMenu = favFocusMenu;
                        ...
                        console.log("update이동!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                        $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                        $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').append(btnokfill);
                        Gigamart.app.shopper_bag.KeyEventActorProvider.updateSubCategoryList();
                    }
        ...
        $.ajax({
            ...
            success     : function(result) {
                ...
                if(obj_length == 1){
                    ...
                    if(totalNewListPage_html < 10 && totalNewListPage_html > 0){
    ...
    pagingOrderedProduct : function(page) {
        ...
        if(currentOrderListPage_html < 10 && currentOrderListPage_html>0){
    ...
    pagingOrderedDtProduct : function(page) {
        ...
        //첫페이지 아닐때
            ...
            if(currentOrderDtListPage != totalOrderDt2ListPage){
                ...
                if(currentOrderDtListPage == Number(totalOrderDt2ListPage)-1){
                    //마지막 리스트갯수가 1개이거나 2개이면 total보여줌
                    if(objTot_len2 == 1 ||objTot_len2 == 2){
                        console.log("마지막 리스트갯수가 1개이거나 2개이면 total보여줌 objTot_len2 : "+objTot_len2);
                        //orderDt2FocusMenu = 0;
                        orderDtTotalPageYN = true;
    ...
    pagingRefProduct : function(page) {
        ...
        if(currentRefListPage_html < 10 && currentRefListPage_html>0){
    ...
    pagingRefDtProduct : function(page) {
        ...
        //첫페이지 아닐때
            ...
            if(currentRefDtListPage != totalRefDt2ListPage){
                ...
                if(currentRefDtListPage == Number(totalRefDt2ListPage)-1){
                    //마지막 리스트갯수가 1개이거나 2개이면 total보여줌
                    if(objTot_len2 == 1 ||objTot_len2 == 2){
                        console.log("마지막 리스트갯수가 1개이거나 2개이면 total보여줌 objTot_len2 : "+objTot_len2);
                        //orderDt2FocusMenu = 0;
    ...
    updateSubCategoryList : function () {
       ...
       console.log("## 찜한상품 페이징 변경 업데이트currentFavListPage : "+currentFavListPage);
       // 변경 해야 할 파라미터 
       ...
       var startIdx     = Number(currentFavListPage) * 8;     // 페이지에서 시작되는 idx
       var endIdx       = (Number(startIdx) + 8);
       var totalPage    = Math.ceil(resultLen / 8);

       //console.log("## 전체 페이지 수 : " + totalPage);

       $('#pageNavi').empty().append("<B>" + Number(Number(currentFavListPage)+1) + "</b> / " + totalPage);
       ...
       // 결과값이 9보다 작으면 결과값 만큼만 상품 리스트를 뿌리고 빈 값으로 나머지를 채워준다.
        ...
        console.log("currentFavListPage_html--->"+currentFavListPage_html);
        if(currentFavListPage_html < 10 && currentFavListPage_html>0){
        ...
        if(requestMyStatus != null && requestMyStatus != '' && requestMyStatus != 'undefined' && requestMyStatus != 0) {  
        console.log("<<<<<<<<<<<<<<<update에서 favFocusMenu>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+favFocusMenu);
            for(var z = 0; z<8; z++){
                $('li[name="fav_menu"]').eq(z).removeClass('focus');
                $('li[name="fav_menu"]:eq('+ z + ') > .dm_bdr').empty();    
            }
            if(verticalFocus == 0){
                if(favFocusMenu ==4 || favFocusMenu==5 ||favFocusMenu==6 ||favFocusMenu==7){
                    favFocusMenu = Number(favFocusMenu)-4;
                }
                console.log("000000000000000000favFocusMenu : "+favFocusMenu);
                $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').append(btnokfill);
            }
            else if(verticalFocus == 1){
                ...
                console.log("111111111111111111favFocusMenu : "+favFocusMenu);
                $('li[name="fav_menu"]').eq(favFocusMenu).addClass('focus');
                $('li[name="fav_menu"]:eq('+ favFocusMenu + ') > .dm_bdr').append(btnokfill);
            }
...
updateSubCategoryNewList : function () {
   ...
   // 결과값이 9보다 작으면 결과값 만큼만 상품 리스트를 뿌리고 빈 값으로 나머지를 채워준다.
    ...
    var currentNewListPage_html = 0;
        ...
        if(currentNewListPage_html < 10 && currentNewListPage_html>0){
...
//최근본상품 페이징 화살표
    ...
    favDetailProduct: function(favDtId) {
        ...
        $.ajax({
            ...
            success     : function(result) {
                ...
                location.href = EXHB_PATH + "product1.html?id="+favDtId+"&categoryDtlCode="+subCate+"&SHOPPER_STATUS=" + SHOPPER_STATUS+"&requestMyStatus=" + MY_STATUS+"&requestMyPage=" + currentFavListPage+"&requestMyMenu=" + favFocusMenu+"&requestMyCnt=" + currentPageCnt+ '&userID='+ userID + '&userName=' + userName;
    ...
    //주문취소
    orderRefProduct: function(ordId) {
        console.log("주문취소 orderId : "+ordId)
        var param = {
                        "order_id" : Number(ordId),
                        "status" : -1
                    };
        $.ajax({
            url         : cmsServerIp + "/BuyerOrderTask/Update/order_status",
            type        : "post",
            dataType    : "json",
            data        : param,
            async       : true,
            ...
            complete : function(result){
                ...
                $('div[name="view_my"]').show();
                $('li[name="nm_menu"]').eq(currentFocusMenu).addClass('focus');
                $('tr[name="ord_menu"]').remove();
                //취소/환불내역 totalcount를 위해
                Gigamart.app.shopper_bag.KeyEventActorProvider.totRef(1);
                //주문내역 totalcount를 위해
                Gigamart.app.shopper_bag.KeyEventActorProvider.totOrder(1);

                //마이페이지 주문내역
                Gigamart.app.shopper_bag.KeyEventActorProvider.selectMyOrderList();
