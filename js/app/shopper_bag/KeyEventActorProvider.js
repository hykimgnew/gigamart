...
App.defineClass('Gigamart.app.shopper_bag.KeyEventActorProvider', {
            ...
            if (keyCode === global.VK_ENTER) {
                ...
                if(chgVolumeFocus == 0) {
                    ...
                    if(cartFocus == 0) {
                        ...
                        location.href = EXHB_PATH + 'order.html?SHOPPER_STATUS=' + SHOPPER_STATUS + '&requestOrderScreen=shopper_bag.html'+ '&userID='+ userID + '&userName=' + userName;
                    }
                    ...
                    if(cartFocus == 1) {
                        ...
                        if($('#ec_list > li[name="ec_li_list"]').size() > 0) {
                            ...
                            $('#span_videoDiv video').remove();
                            appConfiguration.localSystem.mute = true; // 음소거 설정
                            location.href = EXHB_PATH + 'order.html?SHOPPER_STATUS=' + SHOPPER_STATUS + '&requestOrderScreen=shopper_bag.html'+ '&userID='+ userID + '&userName=' + userName + '&startScreen=2';   
                        } else {
            ...
            // **************************************************
                ...
                if(keyCode === global.VK_LEFT) {
                   ...
                   //마트는 지금?
                        ...
                        //쇼퍼추천세트 갯수가 0이면 쇼퍼list로 이동
                        if($('li[name="pl_menu"]').length == 0){
                        	currentFocusList = 0;
                            currentFocusMenu2 = 0;
                            $('li[name="sbl_list"]').eq(currentFocusMenu).addClass('focus');
                        }
    ...
    pagingOrderedProduct : function(page) {
        ...
        console.log("12312312312312"+arrShopperRcList[Number((page*6)+i)]);
        var obj = arrShopperRcList[Number((page*6)+i)];
        if(obj != undefined){
        	//쇼퍼추천세트list
	        for(var i=0 ; i < 6 ; i++) {
	            //makeShopperProduct();
